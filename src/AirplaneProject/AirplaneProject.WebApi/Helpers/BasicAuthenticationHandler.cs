...
namespace AirplaneProject.WebApi.Helpers
{
	...
	{
		private readonly IBasicSecurityService securityService;
		...
		protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
		{
			if (!Request.Headers.ContainsKey("Authorization"))
				return AuthenticateResult.Fail("Cabeçalho de autorização não informado");
			...
			var claims = new[] {
				new Claim(ClaimTypes.NameIdentifier, user.Chave),
				new Claim(ClaimTypes.Name, user.Chave),
