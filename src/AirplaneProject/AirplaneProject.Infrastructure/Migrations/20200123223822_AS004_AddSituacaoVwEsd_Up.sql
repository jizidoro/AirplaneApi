...
SELECT Esd.Id
	  ...
	  + Agente.Codigo + ' ' + Situacao.Codigo + ' '  + ' ' + dbo.RemoveDiacritics(Esd.Descricao) + ' '
	  + (CASE WHEN Esd.Alarme is null THEN ' ' ELSE dbo.RemoveDiacritics(Esd.Alarme)END) as DescricaoCodigo
  FROM Esds as Esd
  ...
  INNER JOIN Situacoes as Situacao ON Situacao.Id = Esd.SituacaoId
