...
SELECT Evento.Id
    ...
    ,Evento.Checksum
    ,Evento.Descricao
    ,Ativo.Codigo + ' ' + Uep.Codigo + ' ' + Sistema.Codigo + ' ' + Camada.Codigo + ' ' + Situacao.Codigo + ' ' + Funcao.Codigo + ' ' 
	  + Solicitante.Chave + ' ' + Autorizador.Chave + ' ' + Executor.Chave + ' ' + Verificador.Chave + ' ' 
	  + Aprovador.Chave + ' ' + dbo.RemoveDiacritics(Evento.Checksum) + ' ' + dbo.RemoveDiacritics(Evento.Descricao) as DescricaoCodigo
  ...
  LEFT JOIN Profissionais as Aprovador ON Aprovador.Id = Evento.AprovadorId
  LEFT JOIN Profissionais as Autorizador ON Autorizador.Id = Evento.AutorizadorId
  LEFT JOIN Profissionais as Verificador ON Verificador.Id = Evento.VerificadorId
GO